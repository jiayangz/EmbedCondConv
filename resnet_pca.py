...
from condconv_vector_1_resnet_pca import CondConv
...
class ResNetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = CondConv(in_planes, planes, kernel_size=3, padding=1, stride=stride, bias=False)
        self.bn1 = get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=planes)
        self.act = get_act_layer(name=act)
        self.conv2 = CondConv(planes, planes, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.conv2((out,vector_weights))
        out = self.bn2(out)
...
class ResNetBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = CondConv(in_planes, planes, kernel_size=1, bias=False)
        self.bn1 = norm_layer(channels=planes)
        self.conv2 = CondConv(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = norm_layer(channels=planes)
        self.conv3 = CondConv(planes, planes * self.expansion, kernel_size=1, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.conv3((out,vector_weights))
        out = self.bn3(out)
...
class ResNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.layer1((x,vector_weights))
        x = self.layer2((x,vector_weights))
        x = self.layer3((x,vector_weights))
        x = self.layer4((x,vector_weights))
